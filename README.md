...
```text
Logging in with phone from .env: +10000000000
...
Logged in as: YourName (@your_username)
...
```powershell
explorer output\exports
```

```powershell
start excel "output\exports\batch_001.csv"
```
...
| --------------------- | ---------------- | --------------------------------------------------------- |
| `API_ID`              | `12345678`       | From [my.telegram.org/apps](https://my.telegram.org/apps) |
| `API_HASH`            | `abc123...`      | 32-character hash from same page                          |
| `PHONE_NUMBER`        | `+10000000000` | Your phone for first login (no spaces)                    |
...
| `*.session` (your login)   | `.env.example`        |
...
| `PhoneNumberInvalidError`          | Use `+10000000000` format — no spaces, correct digits          |
